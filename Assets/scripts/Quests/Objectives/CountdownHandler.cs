...
public class CountdownHandler : IQuestObjectiveHandler
{
    ...
    )
    {
        ...
        float progressBefore = state.ObjectivesProgress.Count > objectiveIndex 
        ...
        {
            Debug.Log($"[CountdownHandler] Progresando misión: '{quest.QuestName}' ({quest.QuestId})\n" +
        ...
        if (state.IsObjectiveCompleted(objectiveIndex, objective))
        {
            Debug.Log($"[CountdownHandler] ✓ Objetivo completado - Misión: {quest.QuestName}, Progreso final: {progressAfter:F2}s / {requiredTime:F2}s");
